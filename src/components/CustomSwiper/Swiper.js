...
export function CustomSwiper() {
    ...
    const getCardColors = () => {
        ...
        currentTheme === "lightMode"
            ? setColors(cardDarkMode)
            : setColors(cardLightMode);
    };
                    ...
                    return (
                        <SwiperSlide key={repository.id}>
                            <a href={repository.html_url} target="_blank">
                                <img
                                    src={`https://github-readme-stats.vercel.app/api/pin/?username=davilacerda&repo=${repository.name}&bg_color=${colors[0]}&border_color=${colors[0]}&title_color=${colors[1]}&text_color=${colors[1]}`}
                                    alt={`GitHub ${repository.name} Stats Card`}
                                />
                            </a>
                        </SwiperSlide>
                    );
